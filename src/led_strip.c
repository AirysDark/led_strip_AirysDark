...
// --------------------------------------------------
// Encode pixel with brightness + order
// --------------------------------------------------
static inline void encode_pixel(uint8_t *dst, rgb_t c, uint8_t br, led_strip_order_t order)
{
    uint8_t r = (c.r * br) >> 8;
    uint8_t g = (c.g * br) >> 8;
    uint8_t b = (c.b * br) >> 8;
...
esp_err_t led_strip_init(led_strip_t *strip)
{
    ...
    ESP_LOGI(TAG, "LED strip initialized (RMT TX, IDF5)");
    return ESP_OK;
}
...
// PIXELS
// --------------------------------------------------
esp_err_t led_strip_set_pixel(led_strip_t *strip, size_t index, rgb_t c)
{
...
esp_err_t led_strip_fill(led_strip_t *strip, size_t start, size_t len, rgb_t c)
{
    CHECK_ARG(strip && strip->buf && start + len <= strip->length);
    for (size_t i = start; i < start + len; i++)
        led_strip_set_pixel(strip, i, c);
    return ESP_OK;
}
